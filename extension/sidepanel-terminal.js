...
(() => {
	 ...
	 */
	window.gstackInjectToTerminal = (text) => {
	...
	async function connect() {
		...
		if (!ptySessionToken) {
			setState(STATE.IDLE, {
				message: "Cannot start: no session token returned",
			});
			return;
		}
	...
	function tryAutoConnect() {
		...
		const tick = () => {
			...
			if (waited > 15000) {
				setState(STATE.IDLE, {
					message: "Browse server not ready. Reload sidebar to retry.",
				});
				return;
			}
			setTimeout(tick, 200);

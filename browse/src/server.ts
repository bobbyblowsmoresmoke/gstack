...
import { safeKill, safeUnlink, safeUnlinkQuiet } from "./error-handling";
...
const AUTH_TOKEN =
	sanitizeAuthToken(process.env.AUTH_TOKEN) || crypto.randomUUID();
...
async function shutdown(exitCode: number = 0) {
	...
	try {
		...
		spawnSync("pkill", ["-f", "terminal-agent\\.ts"], {
			stdio: "ignore",
			timeout: 3000,
		});
	} catch (err: any) {
					...
					start(controller) {
						...
						const unsubscribe = subscribe((entry) => {
							try {
								...
								console.debug(
									"[browse] Activity SSE stream error, unsubscribing:",
									err.message,
								);
								unsubscribe();
							}
						...
						const heartbeat = setInterval(() => {
							try {
								...
								console.debug(
									"[browse] Activity SSE heartbeat failed:",
									err.message,
								);
					...
					start(controller) {
						...
						const notify: InspectorSubscriber = (event) => {
							try {
								...
								console.debug(
									"[browse] Inspector SSE stream error:",
									err.message,
								);
						...
						const heartbeat = setInterval(() => {
							try {
								...
								console.debug(
									"[browse] Inspector SSE heartbeat failed:",
									err.message,
								);
								clearInterval(heartbeat);
								inspectorSubscribers.delete(notify);

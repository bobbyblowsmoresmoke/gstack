...
async function sendCommand(
	...
	try {
		...
		if (
			...
			if (_globalFlags?.proxyUrl)
				restartEnv.BROWSE_PROXY_URL = _globalFlags.proxyUrl;
			if (_globalFlags?.headed) restartEnv.BROWSE_HEADED = "1";
			if (_globalFlags?.configHash)
				restartEnv.BROWSE_CONFIG_HASH = _globalFlags.configHash;
		...
		if (existingState.xvfbPid && existingState.xvfbStartTime) {
			try {
				...
				cleanupXvfb({
					...
					startTime: String(existingState.xvfbStartTime),

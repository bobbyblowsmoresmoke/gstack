...
describe("filterArgs — privacy filtering", () => {
	...
	it("preserves fill value for non-password fields", () => {
		expect(filterArgs("fill", ["#email", "user@test.com"])).toEqual([
			"#email",
			"user@test.com",
		]);
	});
	...
	it("redacts cookie values", () => {
		expect(filterArgs("cookie", ["session_id=abc123"])).toEqual([
			"session_id=[REDACTED]",
		]);
	});

	it("redacts sensitive URL query params", () => {
		const result = filterArgs("goto", [
			"https://example.com?api_key=secret&page=1",
		]);
...
describe("subscribe", () => {
	...
	it("stops receiving after unsubscribe", async () => {
		...
		expect(received.filter((e) => e.command === "should-not-see").length).toBe(
			0,
		);
	});
});

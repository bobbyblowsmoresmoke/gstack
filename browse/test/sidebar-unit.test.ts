...
describe("sanitizeExtensionUrl", () => {
	test("passes valid http URL", () => {
		expect(sanitizeExtensionUrl("http://example.com")).toBe(
			"http://example.com/",
		);
	});
	...
	test("rejects chrome-extension:// URLs", () => {
		expect(
			sanitizeExtensionUrl("chrome-extension://abcdef/popup.html"),
		).toBeNull();
	});
	...
	test("preserves port numbers", () => {
		expect(sanitizeExtensionUrl("http://localhost:3000/api")).toBe(

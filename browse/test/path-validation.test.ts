...
describe("upload command path validation", () => {
	const src = readFileSync(
		join(__dirname, "..", "src", "write-commands.ts"),
		"utf-8",
	);

	it("validates upload paths with isPathWithin", () => {
		const uploadBlock = src.slice(
			src.indexOf("case 'upload'"),
			src.indexOf("case 'dialog-accept'"),
		);
...
describe("validateOutputPath — symlink resolution", () => {
	it("blocks symlink inside /tmp pointing outside safe dirs", () => {
		const linkPath = join(
			tmpdir(),
			"test-output-symlink-" + Date.now() + ".png",
		);
		try {
	...
	it("blocks new file in symlinked directory pointing outside", () => {
		...
		try {
			...
			expect(() => validateOutputPath(join(linkDir, "evil.png"))).toThrow(
				/Path must be within/,
			);
		} finally {
			try {

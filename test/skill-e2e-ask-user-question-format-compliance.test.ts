...
describeE2E("AskUserQuestion format compliance (gate)", () => {
	test("first AskUserQuestion from /plan-ceo-review contains all 7 mandated format elements", async () => {
		...
		try {
			...
			while (Date.now() - start < budgetMs) {
				...
				const hasCursor =
					isNumberedOptionListVisible(cursorTail) &&
			...
			if (gaps.length > 0) {
				...
				throw new Error(
					`AskUserQuestion format compliance FAILED — missing ${gaps.length} mandated field(s):\n` +
						gaps
							.map((g) => `  - ${g.field} (regex: ${g.re.source})`)
							.join("\n") +
						`\n--- captured (last 3KB) ---\n${tail}`,
				);
			}

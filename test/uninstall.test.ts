...
describe("gstack-uninstall", () => {
	...
	describe("integration tests with mock layout", () => {
		...
		beforeEach(() => {
			...
			fs.mkdirSync(path.join(mockHome, ".claude", "skills", "gstack"), {
				recursive: true,
			});
			fs.writeFileSync(
				path.join(mockHome, ".claude", "skills", "gstack", "SKILL.md"),
				"test",
			);
			...
			fs.symlinkSync(
				"gstack/review",
				path.join(mockHome, ".claude", "skills", "review"),
			);
			fs.symlinkSync(
				"gstack/ship",
				path.join(mockHome, ".claude", "skills", "gstack-ship"),
			);

			// Create a non-gstack symlink (should NOT be removed)
			fs.mkdirSync(path.join(mockHome, ".claude", "skills", "other-tool"), {
				recursive: true,
			});

			// Create state directory
			fs.mkdirSync(path.join(mockHome, ".gstack", "projects"), {
				recursive: true,
			});
		...
		test("--force removes global Claude skills and state", () => {
			...
			expect(
				fs.existsSync(path.join(mockHome, ".claude", "skills", "gstack")),
			).toBe(false);

			// Per-skill symlinks pointing into gstack/ should be removed
			expect(
				fs.existsSync(path.join(mockHome, ".claude", "skills", "review")),
			).toBe(false);
			expect(
				fs.existsSync(path.join(mockHome, ".claude", "skills", "gstack-ship")),
			).toBe(false);

			// Non-gstack tool should still exist
			expect(
				fs.existsSync(path.join(mockHome, ".claude", "skills", "other-tool")),
			).toBe(true);
		...
		test("--keep-state preserves state directory", () => {
			...
			expect(fs.existsSync(path.join(mockHome, ".gstack", "config.json"))).toBe(
				true,
			);
		});

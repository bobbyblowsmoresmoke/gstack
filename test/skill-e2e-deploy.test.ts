...
describeIfSelected(
	...
	() => {
		...
		beforeAll(() => {
			firstRunDir = fs.mkdtempSync(
				path.join(os.tmpdir(), "skill-e2e-land-first-run-"),
			);
			const run = (cmd: string, args: string[]) =>
				spawnSync(cmd, args, {
					cwd: firstRunDir,
			...
			copyDirSync(
				path.join(ROOT, "land-and-deploy"),
				path.join(firstRunDir, "land-and-deploy"),
			);
		});
	...
	() => {
		...
		beforeAll(() => {
			reviewDir = fs.mkdtempSync(
				path.join(os.tmpdir(), "skill-e2e-land-review-"),
			);
			const run = (cmd: string, args: string[]) =>
			...
			copyDirSync(
				path.join(ROOT, "land-and-deploy"),
				path.join(reviewDir, "land-and-deploy"),
			);
		});

	...
	beforeAll(() => {
		...
		const learnings = [
			...
			},
			{
				...
				insight:
					"Redis cache not invalidated on deploy causes stale data for 5 minutes.",
				confidence: 7,
				source: "observed",
				ts: new Date().toISOString(),
			...
			},
			{
				...
				insight:
					"User wants rubocop to run before every commit, no exceptions.",
				confidence: 10,
				source: "user-stated",
				ts: new Date().toISOString(),
			},
			{
				skill: "qa",
				type: "operational",
				key: "test-timeout-flag",
				insight:
					"bun test requires --timeout 30000 for E2E tests in this project.",
				confidence: 9,
				source: "observed",
				ts: new Date().toISOString(),
			},
...
- Just show the learnings and summarize what you found.`,
			...
			const mentionsNPlusOne =
				output.includes("n-plus-one") || output.includes("n+1");
			const mentionsCache =
				output.includes("stale") || output.includes("cache");

		...
		testIfSelected(
			...
			async () => {
				...
				const resp = await api("/sidebar-command", {
					...
					body: JSON.stringify({
						message:
							"Go to https://example.com. Read the page title. Add a 4px solid orange outline to the h1 element.",
				...
				if (!doneEntry || entries.length === 0) {
					...
					console.log(
						"QUEUE:",
						queueContent.length > 0 ? "has entries" : "empty",
					);
				}
				...
				const allEntryText = entries
					.map(
						(e: any) => `${e.text || ""} ${e.input || ""} ${e.message || ""}`,
					)
					.join(" ");
				const navigatedToTarget =
					allEntryText.includes("example.com") ||
					allEntryText.includes("Example Domain");
				if (!navigatedToTarget) {
					console.log(
						"ALL ENTRY TEXT (first 2000):",
						allEntryText.slice(0, 2000),
					);
				}
